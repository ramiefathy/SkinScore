...
export const nrsPruritusTool: Tool = {
  ...
  description: "The NRS for pruritus is a unidimensional, patient-reported outcome measure designed to quantify the intensity of itch, a subjective and multifactorial symptom prevalent in many dermatological and systemic diseases. The rationale for the NRS was to provide a simple, reliable, and valid tool for both clinical and research settings, allowing for standardized assessment and monitoring of pruritus severity. The NRS typically asks patients to rate their itch intensity over a specified recall period (commonly the past 24 hours) on an 11-point scale from 0 (no itch) to 10 (worst imaginable itch). The original development of the NRS is not attributed to a single publication, as it evolved alongside similar scales such as the Visual Analogue Scale (VAS) and Verbal Rating Scale (VRS). Its psychometric properties and validation in chronic pruritus populations have been robustly established, including in the prospective multicenter study by Phan et al. (2012), which confirmed high reliability and concurrent validity for the NRS, VAS, and VRS in 471 patients with chronic itch. The American Academy of Dermatology, in its joint guidelines with the National Psoriasis Foundation, recognizes the NRS as a valid instrument for pruritus assessment in psoriasis and other dermatoses.",
  sourceType: 'Research',
  icon: CircleDot,
  formSections: [
